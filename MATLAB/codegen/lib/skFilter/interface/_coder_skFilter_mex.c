...
void mexFunction(int32_T nlhs, mxArray *plhs[], int32_T nrhs,
                 const mxArray *prhs[])
{
  mexAtExit(&skFilter_atexit);
  /* Module initialization. */
  skFilter_initialize();
  /* Dispatch the entry-point. */
  unsafe_skFilter_mexFunction(nlhs, plhs, nrhs, prhs);
  /* Module termination. */
  skFilter_terminate();
}
                                 ...
                                 const mxArray *prhs[1])
{
  ...
  /* Call the function. */
  skFilter_api(prhs[0], &outputs);
  /* Copy over outputs to the caller. */
  emlrtReturnArrays(1, &plhs[0], &outputs);
}

/*
 * File trailer for _coder_skFilter_mex.c
 *
 * [EOF]
 */
