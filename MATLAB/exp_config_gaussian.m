function par = exp_config_gaussian()
...
%
% p = gamma+(1-gamma-lambda).*(1+erf((x-alpha)./sqrt(2*beta.^2)))/2;,
%
...
% psychometric function and 1, alpha and beta are the threshold and spread 
...
par.x0 = 25;    % the initial signal strength
par.x_lim = [-30 30];   % the limits to the signal strength

par.alpha = struct(...
    'limits',[-20 20],...       %range of the parameter space for alpha
...
par.lambda = struct(...
    ...
    'N',5,...                 %number of lambda values. If this value is set to 1, then the first element of lambda.limits would be the assumed lambda and the lambda parameter is not estimated.
    'scale','lin',...         %the linear or log spacing. Choose between 'lin' and 'log'.
