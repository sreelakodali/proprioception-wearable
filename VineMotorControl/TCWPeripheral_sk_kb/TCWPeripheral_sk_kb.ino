...
BLEService ledService("19B10000-E8F2-537E-4F6C-D104768eA121");  // Bluetooth® Low Energy LED Service
BLEByteCharacteristic switchCharacteristic("19B10001-E8F2-537E-4F6C-D104768A1214", BLERead | BLEWrite); // This should be the same for all winches (do not modify)


# define onLedPin LEDG
# define bleLedPin LEDB
...
// const int bleLedPin = 3;    // LED indicator: nano is connected to central
// const int actLedPin = 4;    // LED indicator: the actuator is forward
// const int failLedPin = 12;  // LED indicator: ble has failed

// define analog pins to send PWM to motor controller
const int pwm0Pin = A6;         // IN1
const int pwm1Pin = A7;         // IN2
...
void loop() {
  digitalWrite(bleLedPin, LOW); // Always default that we're not connected to BLE
  ...
  if (central) {
    digitalWrite(bleLedPin, HIGH); // indicate
