...
for i in list(range(0, len(subjects))):
	...
	for f in os.listdir(CONST.PATH_LAPTOP + p):
		...
		if f.startswith('forceAtClick_') and f.endswith('.csv'):
			data2 = pd.read_csv(CONST.PATH_LAPTOP + p + f, delimiter = "\n", header=None).astype(float)
...
# 		# for this subject, extract the ones of that condition, 
	...
	arr_HnV[i] = [s for _, s in sorted(zip( [target[k] for k in test_HnV], [subjectAttempt[k] for k in test_HnV]))]
	#print(arr_HnV)
	arr_HV[i] = [s for _,s in sorted(zip( [target[k] for k in test_HV], [subjectAttempt[k] for k in test_HV]))]
	arr_nHnV[i] = [s for _, s in sorted(zip( [target[k] for k in test_nHnV], [subjectAttempt[k] for k in test_nHnV]))]
	arr_nHV[i] = [s for _, s in sorted(zip( [target[k] for k in test_nHV], [subjectAttempt[k] for k in test_nHV]))]


f = open('subjectAttempts_HnV_16.csv','w+', encoding='UTF8', newline='')
writer = csv.writer(f)
writer.writerows(arr_HnV)
f.close()

f = open('subjectAttempts_HV_16.csv','w+', encoding='UTF8', newline='')
writer = csv.writer(f)
writer.writerows(arr_HV)
f.close()

f = open('subjectAttempts_nHnV_16.csv','w+', encoding='UTF8', newline='')
writer = csv.writer(f)
writer.writerows(arr_nHnV)
f.close()

f = open('subjectAttempts_nHV_16.csv','w+', encoding='UTF8', newline='')
writer = csv.writer(f)
